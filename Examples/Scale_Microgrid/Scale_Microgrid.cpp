...
int main(int argc, char const *argv[])
{
    ...
    std::vector<size_t> compDataSizes = {2,4,8};
    //All data generated with abstol=1e-12 and reltol=1e-12 with ode23tb
    if (orgparams && compDataSizes.end() != std::find(compDataSizes.begin(), compDataSizes.end(), Nsize))
    {   
        //get the cmake source dir from the cmake configuration
        std::string base_path = XSTRING(SOURCE_ROOT);

        base_path += "/Examples/Scale_Microgrid/microgrid_N";
        base_path += std::to_string(Nsize);
        base_path += ".txt";
