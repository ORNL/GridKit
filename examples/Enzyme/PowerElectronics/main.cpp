 ...
 *
 * TODO: Move automatic differentiation inside GridKit and convert this into a
 * unit test.
 */
...
void evaluateResidual(std::vector<double> y_, std::vector<double> f_)
{
    ...
    double ilq_star = F_ * y_[15] + wb_ * Cf_ * y_[12]
...
int main()
{
    ...
    for (int idy = 0; idy < dg->size(); ++idy)
    {
        ...
        {
            if (std::abs(jac_autodiff.getValue(idx, idy)
                         - jac_ref_dense.getValue(idx, idy))
                > std::numeric_limits<double>::epsilon())
            {
                fail++;
                if (verbose)
                {
                    std::cout << "Result incorrect at line = " << idy
                              << ", column = " << idx << "\n";
                }
            }
        }
