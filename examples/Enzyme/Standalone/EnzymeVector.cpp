...
/**
 * @brief Standalone example that computes the Jacobian of a vector-valued
 * function by automatic differentiation via Enzyme.
 *
...
void dsquare(std::vector<double> x, std::vector<double> y, DenseMatrix& dy)
{
    ...
    for (int idy = 0; idy < y.size(); ++idy)
    {
        ...
        // Autodiff
        __enzyme_fwddiff(
            (void*) square, enzyme_dup, x, v, enzyme_dupnoneed, y, &d_y);
...
int main()
{
    ...
    for (int idy = 0; idy < sq.size(); ++idy)
    {
        for (int idx = 0; idx < x.size(); ++idx)
        {
            if (std::abs(dsq.getValue(idx, idy) - dsq_ref.getValue(idx, idy))
                > std::numeric_limits<double>::epsilon())
            {
                ...
                if (verbose)
                {
                    std::cout << "Result incorrect at line = " << idy
                              << ", column = " << idx << "\n";
                    std::cout << "x = " << x[idx] << ", x^2 = " << sq[idx]
                              << ", d(x^2)/dx = " << dsq.getValue(idx, idy)
                              << "\n";
                }
            }
        }
