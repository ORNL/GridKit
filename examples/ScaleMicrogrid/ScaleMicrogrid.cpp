...
// Include solution keys for the three test cases N = (2, 4, 8) plus tolerances
// here:
#include "SolutionKeys.hpp"

static int
test(index_type Nsize, real_type test_tolerance, bool error_tol = false);

/**
 * @brief Run Scale Microgrid test cases of N = (2,4,8) and check for
 * correctness.
 *
 * @param argc unused
 * @param argv unsued
...
int test(index_type Nsize, real_type error_tol, bool debug_output)
{
    ...
    auto* sysmodel = new PowerElectronicsModel<real_type, index_type>(
        rel_tol, abs_tol, use_jac, SCALE_MICROGRID_MAX_STEPS);
    ...
    // Every Bus has the same virtual resistance. This is due to the numerical
    // stability as mentioned in the paper.
    real_type RN = 1.0e4;
    ...
    std::vector<GridKit::DistributedGeneratorParameters<real_type, index_type>>
        DGParams_list(2 * Nsize, DG_parms2);

    DGParams_list[0] = DG_parms1;
    DGParams_list[1] = DG_parms1;

    // line vector params
    // Every odd line has the same parameters and every even line has the same
    // parameters
    ...
    auto* dg_ref = new DistributedGenerator<real_type, index_type>(
        0, DGParams_list[0], true);
    // ref motor
    ...
    {
        // current DG to add
        auto* dg = new DistributedGenerator<real_type, index_type>(
            model_id++, DGParams_list[i], false);
        // ref motor
    ...
    {
        // line
        auto* line_model = new MicrogridLine<real_type, index_type>(
            model_id++, rline_list[i], Lline_list[i]);
        // ref motor
    ...
    {
        auto* load_model = new MicrogridLoad<real_type, index_type>(
            model_id++, rload_list[i], Lload_list[i]);
        // ref motor
    ...
    {
        auto* virDQbus_model =
            new MicrogridBusDQ<real_type, index_type>(model_id++, RN);
    ...
    // Create Intial points for states. Every state is to specified to the zero
    // intially
    ...
    for (index_type i = 0; i < 2 * Nsize; i++)
    {
        ...
        sysmodel->yp()[13 * i - 1 + 5] =
            DGParams_list[i].Kpv_ * DGParams_list[i].Vn_;
        sysmodel->yp()[13 * i - 1 + 7] =
            (DGParams_list[i].Kpc_ * DGParams_list[i].Kpv_
             * DGParams_list[i].Vn_)
            / DGParams_list[i].Lf_;
    }

    // since the intial P_com = 0, the set the intial vector to the reference
    // frame
    ...
    auto* idas =
        new AnalysisManager::Sundials::Ida<real_type, index_type>(sysmodel);
    ...
    for (index_type i = 0; i < true_vec->size(); i++)
    {
        ...
        if (debug_output)
            std::cout << i << " : "
                      << abs(true_vec->at(i) - yfinal[i]) / abs(true_vec->at(i))
                      << "\n";
