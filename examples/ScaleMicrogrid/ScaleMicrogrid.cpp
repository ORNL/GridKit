...
int test(index_type Nsize, real_type error_tol, bool debug_output)
{
    ...
    auto* sysmodel =
        new PowerElectronicsModel<real_type, index_type>(rel_tol, abs_tol, use_jac, SCALE_MICROGRID_MAX_STEPS);
    ...
    auto* dg_ref = new DistributedGenerator<real_type, index_type>(0, DGParams_list[0], true);
    // ref motor
    ...
    {
        // current DG to add
        auto* dg = new DistributedGenerator<real_type, index_type>(model_id++, DGParams_list[i], false);
        // ref motor
    ...
    {
        // line
        auto* line_model = new MicrogridLine<real_type, index_type>(model_id++, rline_list[i], Lline_list[i]);
        // ref motor
    ...
    {
        auto* load_model = new MicrogridLoad<real_type, index_type>(model_id++, rload_list[i], Lload_list[i]);
        // ref motor
    ...
    for (index_type i = 0; i < 2 * Nsize; i++)
    {
        ...
        sysmodel->yp()[13 * i - 1 + 7] =
            (DGParams_list[i].Kpc_ * DGParams_list[i].Kpv_ * DGParams_list[i].Vn_) / DGParams_list[i].Lf_;
    }
