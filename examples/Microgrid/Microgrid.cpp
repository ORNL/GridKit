...
int main(int argc, char const* argv[])
{
    ...
    // Create model
    GridKit::PowerElectronicsModel<double, size_t>* sysmodel =
        new GridKit::PowerElectronicsModel<double, size_t>(
            rel_tol, abs_tol, use_jac, max_step_amount);
    ...
    size_t Nsize = 2;
    //							DGs	+
    //- refframe	   Lines +				Loads
    size_t vec_size_internals =
        13 * (2 * Nsize) - 1 + (2 + 4 * (Nsize - 1)) + 2 * Nsize;
    //							\omegaref + BusDQ
    ...
    // dg 1
    GridKit::DistributedGenerator<double, size_t>* dg1 =
        new GridKit::DistributedGenerator<double, size_t>(0, parms1, true);
    // ref motor
    ...
    // dg 2
    GridKit::DistributedGenerator<double, size_t>* dg2 =
        new GridKit::DistributedGenerator<double, size_t>(1, parms1, false);
    // ref motor
    ...
    // dg 3
    GridKit::DistributedGenerator<double, size_t>* dg3 =
        new GridKit::DistributedGenerator<double, size_t>(2, parms2, false);
    // ref motor
    ...
    // dg 4
    GridKit::DistributedGenerator<double, size_t>* dg4 =
        new GridKit::DistributedGenerator<double, size_t>(3, parms2, false);
    // ref motor
    ...
    // line 1
    GridKit::MicrogridLine<double, size_t>* l1 =
        new GridKit::MicrogridLine<double, size_t>(4, rline1, Lline1);
    // ref motor
    ...
    // line 2
    GridKit::MicrogridLine<double, size_t>* l2 =
        new GridKit::MicrogridLine<double, size_t>(5, rline2, Lline2);
    // ref motor
    ...
    // line 3
    GridKit::MicrogridLine<double, size_t>* l3 =
        new GridKit::MicrogridLine<double, size_t>(6, rline3, Lline3);
    // ref motor
    ...
    // load 1
    GridKit::MicrogridLoad<double, size_t>* load1 =
        new GridKit::MicrogridLoad<double, size_t>(7, rload1, Lload1);
    // ref motor
    ...
    // load 2
    GridKit::MicrogridLoad<double, size_t>* load2 =
        new GridKit::MicrogridLoad<double, size_t>(8, rload2, Lload2);
    // ref motor
    ...
    // Virtual PQ Buses
    GridKit::MicrogridBusDQ<double, size_t>* bus1 =
        new GridKit::MicrogridBusDQ<double, size_t>(9, RN);

    bus1->setExternalConnectionNodes(0, dqbus1);
    bus1->setExternalConnectionNodes(1, dqbus1 + 1);
    sysmodel->addComponent(bus1);

    GridKit::MicrogridBusDQ<double, size_t>* bus2 =
        new GridKit::MicrogridBusDQ<double, size_t>(10, RN);

    bus2->setExternalConnectionNodes(0, dqbus2);
    bus2->setExternalConnectionNodes(1, dqbus2 + 1);
    sysmodel->addComponent(bus2);

    GridKit::MicrogridBusDQ<double, size_t>* bus3 =
        new GridKit::MicrogridBusDQ<double, size_t>(11, RN);

    bus3->setExternalConnectionNodes(0, dqbus3);
    bus3->setExternalConnectionNodes(1, dqbus3 + 1);
    sysmodel->addComponent(bus3);

    GridKit::MicrogridBusDQ<double, size_t>* bus4 =
        new GridKit::MicrogridBusDQ<double, size_t>(12, RN);
    ...
    sysmodel->yp()[12 + 7] =
        (parms1.Kpc_ * parms1.Kpv_ * parms1.Vn_) / parms1.Lf_;
    for (size_t i = 2; i < 4; i++)
    {
        sysmodel->yp()[13 * i - 1 + 3] = parms2.Vn_;
        sysmodel->yp()[13 * i - 1 + 5] = parms2.Kpv_ * parms2.Vn_;
        sysmodel->yp()[13 * i - 1 + 7] =
            (parms2.Kpc_ * parms2.Kpv_ * parms2.Vn_) / parms2.Lf_;
    }
    ...
    AnalysisManager::Sundials::Ida<double, size_t>* idas =
        new AnalysisManager::Sundials::Ida<double, size_t>(sysmodel);
    ...
    std::vector<double> true_vec{
        2.297543153595780e+04,  1.275311524125022e+04,  3.763060183116022e-02,
        -2.098153459325261e-02, 1.848285659119097e-02,  -1.563291404944864e-04,
        6.321941907011718e+01,  -2.942264300846256e+01, 3.634209302905854e+02,
        -2.668928293656362e-06, 6.321941919221522e+01,  -3.509200178595996e+01,
        -7.555954467454730e-03, 2.297580486511343e+04,  8.742028429066131e+03,
        3.710079564796484e-02,  -1.421122598056797e-02, 1.874079517807597e-02,
        -9.891304812687215e-05, 6.232933298360234e+01,  -1.796494061423331e+01,
        3.686353885026506e+02,  3.465673854181523e-05,  6.232933406188410e+01,
        -2.371564475187742e+01, -8.273939686941580e-02, 1.727775042678524e+04,
        1.649365247247288e+04,  3.116555157570849e-02,  -2.985990066758010e-02,
        2.250012115906506e-02,  -2.643873146501096e-04, 4.861823510250247e+01,
        -4.088592755441309e+01, 3.552597163751238e+02,  -1.496407194199739e-04,
        4.861823504694532e+01,  -4.642797132602495e+01, -8.445727984408551e-02,
        1.727723725566433e+04,  9.182386962936238e+03,  3.024959333190777e-02,
        -1.617250828202081e-02, 2.318056864131751e-02,  -1.295918667730514e-04,
        4.718938244522050e+01,  -1.935782085675469e+01, 3.662262287803608e+02,
        1.076423957830039e-04,  4.718938116520511e+01,  -2.507094256286497e+01,
        -1.881248349415025e+01, 2.114714832305742e+01,  4.329946674909793e+01,
        -3.037887936225145e+00, -4.487023117352992e+01, 2.895883729832657e+01,
        8.199613345691378e+01,  -5.623856502948122e+01, 1.327498499660322e+02,
        -8.228065162347022e+01, 3.119995747945993e+02,  3.576922945168803e+02,
        -5.850795361581618e+00, 3.641193316268954e+02,  -8.846325267612976e+00,
        3.472146752739036e+02,  -3.272400970143252e+01, 3.604108939430972e+02,
    ...
    for (size_t i = 0; i < true_vec.size(); i++)
    {
        printf("%lu : %e ,\n",
               i,
               abs(true_vec[i] - yfinial[i]) / abs(true_vec[i]));
    }
