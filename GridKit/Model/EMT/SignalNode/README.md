...
Signal nodes provide scalar connection points between EMT components.
Components attach external inputs to signal nodes and assign internal outputs
to signal nodes through `ComponentSignals`.
...
without owning the producing model. A fully bound signal node additionally
...
contributions into the owner's residual row. Three fully bound signal nodes
