...
--------------- | ------ | ---------
`evaluateInternalEquation(y, yp, y_ext, yp_ext, f)` | $\mathbf{f}$ | Internal rows of the model
`evaluateExternalEquation(y, yp, y_ext, yp_ext, f_ext)` | $\mathbf{f}^\text{ext}$ | Accumulated into internal rows of a connected model
...
struct ModelWrapper<ModelT, Equation::Internal>
{
  ...
  static void eval(ModelT*        model,
                   ...
                   ScalarT*       f)
  {
    model->evaluateInternalEquation(y, yp, y_ext, yp_ext, f);
  }
};
```

The `Equation::External` specialization mirrors it and calls
`evaluateExternalEquation`.
...
struct SparseJacobian
{
                   ...
                   const RealT    scaling = 1.0)
  {
    ...
    for (size_t var_i = 0; var_i < n_var; ++var_i)
    {
      ...
      void* residual = (void*) ModelWrapper<ModelT, equation>::eval;
