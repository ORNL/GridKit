  ...
  namespace EMT
  {
    namespace Controller
    {
      ...
      enum class IeeestExternalVariables : size_t
      {
        ...
        OMEGA, ///< Absolute per-unit machine speed (alternative to U)
        VCT,   ///< Compensated voltage magnitude [p.u.]
        MAXIMUM,
      };
      ...
      private:
        ...
        __attribute__((always_inline)) inline ScalarT cutoutGate(const ScalarT& voltage) const
        {
          return (ONE<RealT> - lower_cutout_ + lower_cutout_ * Math::sigmoid(voltage - Vcl_))
                 * (ONE<RealT> - upper_cutout_ + upper_cutout_ * Math::sigmoid(Vcu_ - voltage));
        }
