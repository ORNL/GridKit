  ...
  namespace EMT
  {
    namespace Controller
    {
      ...
      int GastPti<scalar_type, index_type>::allocate()
      {
        ...
        this->allocateExternalVectors(static_cast<IdxT>(GastPtiExternalVariables::MAXIMUM), 0);
        ...
        {
          signals_.template getSignal<GastPtiInternalVariables::PMECH>()->set(
              &y[PMECH],
       ...
       * parameter relationships, machine/component bases, the required
      ...
      int GastPti<scalar_type, index_type>::verify() const
      {
        ...
        const RealT component_power_base = trate_provided_ ? va_component_base_ : va_machine_base_;
        const bool  valid_component_base = std::isfinite(component_power_base)
                                          && component_power_base > ZERO<RealT>;
        const bool valid_machine_base =
            std::isfinite(va_machine_base_) && va_machine_base_ > ZERO<RealT>;
        if (trate_provided_)
        {
        ...
        check(valid_machine_base, "machine power base must be finite and positive");

        if (valid_component_base && valid_machine_base)
        {
          const RealT machine_to_component = va_machine_base_ / component_power_base;
          const RealT component_to_machine = component_power_base / va_machine_base_;
          check(std::isfinite(machine_to_component)
                    && machine_to_component > ZERO<RealT>
                    && std::isfinite(component_to_machine)
                    && component_to_machine > ZERO<RealT>,
                "machine/component power-base conversion ratios must be finite and positive");
        }
      ...
      int GastPti<scalar_type, index_type>::initialize(const std::map<Outputs, RealT>& outputs)
      {
        ...
        if (!trate_provided_)
        {
          va_component_base_ = va_machine_base_;
        }
        ...
        const ScalarT pmech0       = toComponentBase(pmech_machine0);
        ...
        const RealT pref0           = toMachineBase(pref_component0);
          ...
          ScalarT*                        f)
      {
        ...
        const ScalarT omega = external[OMEGA] - ONE<RealT>;
        const ScalarT pref  = toComponentBase(external[PREF]);
       ...
       *
       * The mechanical-power output is published on the machine base and the
       ...
       * response defaults. An omitted rating is resolved from the current machine
      ...
      /**
       * @brief Convert a machine-base power to GASTPTI component base
       *
       * @param[in] value Quantity on the machine base.
      ...
      {
        return value * (va_machine_base_ / va_component_base_);
      }

      /**
       * @brief Convert a component-base power to the machine base
       *
       * @param[in] value Quantity on the component base.
       * @return The same quantity on the machine base.
       */
      template <typename scalar_type, typename index_type>
      auto GastPti<scalar_type, index_type>::toMachineBase(RealT value) const -> RealT
      {
        return value * (va_component_base_ / va_machine_base_);
      }
