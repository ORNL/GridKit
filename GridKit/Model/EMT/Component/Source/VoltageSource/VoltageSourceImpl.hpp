  ...
  namespace EMT
  {
    ...
    int VoltageSource<scalar_type, index_type>::allocate()
    {
      ...
      for (IdxT phase = 0; phase < 3; ++phase)
        this->bindSignal(u_port_[static_cast<size_t>(phase)], 3 + phase);
      if (yfit_.has_value())
      {
      ...
      this->allocateExternalVectors(static_cast<IdxT>(VoltageSourceExternalVariables::MAXIMUM), 0);
     ...
     */
    template <typename scalar_type, typename index_type>
    void VoltageSource<scalar_type, index_type>::assignOutput(Outputs output, SignalT* signal)
    {
      const auto phase = static_cast<IdxT>(output);
      if (output >= Outputs::SIZE || signal == nullptr)
        throw std::invalid_argument("Invalid VoltageSource output");
      signal->claimProducer();
      signal->setComputed(
          [this, phase]
          { return yfit_ ? yfit_->output(phase) : y_.getData()[3 + phase]; },
          [this, phase](typename SignalT::GradientT& gradient, RealT scale)
          {
            if (yfit_)
              yfit_->appendOutputGradient(phase, gradient, scale);
            else
              gradient.emplace_back(this->getVariableIndex(3 + phase), scale);
          });
    }

    template <typename scalar_type, typename index_type>
    int VoltageSource<scalar_type, index_type>::initialize(const std::map<Outputs, RealT>& outputs)
    {
      ...
      if (!yfit_)
        for (const auto& [output, value] : outputs)
          y[3 + static_cast<size_t>(output)] = static_cast<ScalarT>(value);
