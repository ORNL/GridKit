...
namespace GridKit
{
  namespace EMT
  {
     ...
     *
     * The open command masks entries to exact zero, which drops them from
     * the discovered pattern, so the system Jacobian structure must be
     * invalidated whenever the command changes.
     *
     * @return int - error code, 0 = success
     */
    template <typename scalar_type, typename index_type>
    int Switch<scalar_type, index_type>::assembleJacobian(RealT y_scale, RealT)
    {
      ...
      using ModelT = Switch<ScalarT, IdxT>;
      ...
      if (y_scale != ZERO<RealT>)
      {
        SparseJacobian<ModelT, Equation::External, Variable::Y>::eval(
            this, n_fext, n_y, rie, vi, y, yp, ye, ype, J_rows_buffer_, J_cols_buffer_, J_vals_buffer_, nnz_, y_scale);
      }
