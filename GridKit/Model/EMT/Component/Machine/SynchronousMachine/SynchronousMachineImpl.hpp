...
namespace GridKit
{
  namespace EMT
  {
    ...
    template <typename scalar_type, typename index_type>
    SynchronousMachine<scalar_type, index_type>::SynchronousMachine(const ModelDataT& data)
     ...
     */
    template <typename scalar_type, typename index_type>
    void SynchronousMachine<scalar_type, index_type>::initializeParameters(const ModelDataT& data)
    {
     ...
     */
    template <typename scalar_type, typename index_type>
    void SynchronousMachine<scalar_type, index_type>::setDerivedParams()
    {
     ...
     */
    template <typename scalar_type, typename index_type>
    int SynchronousMachine<scalar_type, index_type>::setGridKitComponentID(IdxT component_id)
    {
     ...
     */
    template <typename scalar_type, typename index_type>
    int SynchronousMachine<scalar_type, index_type>::allocate()
    {
      if (!allocated_)
      {
      ...
      this->allocateExternalVectors(static_cast<IdxT>(SynchronousMachineExternalVariables::MAXIMUM), 3);
      signals_.registerExternalVariableNodes(*this);
      this->setExternalResidualNode(0, signals_.template getAttachedSignalNode<SynchronousMachineExternalVariables::VA>());
      this->setExternalResidualNode(1, signals_.template getAttachedSignalNode<SynchronousMachineExternalVariables::VB>());
      this->setExternalResidualNode(2, signals_.template getAttachedSignalNode<SynchronousMachineExternalVariables::VC>());

      // Set output signals
      if (signals_.template isAssigned<SynchronousMachineInternalVariables::OMEGA>())
      {
        auto* y = y_.getData();
        signals_.template getSignalNode<SynchronousMachineInternalVariables::OMEGA>()->set(&y[1], &(this->getVariableIndex(1)));
      }
     ...
     */
    template <typename scalar_type, typename index_type>
    int SynchronousMachine<scalar_type, index_type>::verify() const
    {
      static constexpr auto EFD = SynchronousMachineExternalVariables::EFD;
      static constexpr auto PM  = SynchronousMachineExternalVariables::PM;

      int error_count = 0;

      if (!signals_.template isAttached<SynchronousMachineExternalVariables::VA>()
          || !signals_.template isAttached<SynchronousMachineExternalVariables::VB>()
          || !signals_.template isAttached<SynchronousMachineExternalVariables::VC>())
      {
        Log::error() << "SynchronousMachine: the bus voltage port is not attached\n";
        ++error_count;
      }

      if (S_ <= 0.0 || V_ <= 0.0 || freq_ <= 0.0 || H_ <= 0.0)
      {
        Log::error() << "SynchronousMachine: the ratings and inertia must be positive\n";
        ++error_count;
      }
      ...
      {
        Log::error() << "SynchronousMachine: the winding inductances must be positive\n";
        ++error_count;
      }

      if (Rfd_ <= 0.0 || R1d_ <= 0.0 || R1q_ <= 0.0 || R2q_ <= 0.0)
      {
        Log::error() << "SynchronousMachine: the rotor winding resistances must be positive\n";
        ++error_count;
      }

      if (Rs_ < 0.0 || Fric_ < 0.0 || S10_ < 0.0 || S12_ < 0.0)
      {
        Log::error() << "SynchronousMachine: the stator resistance, friction factor, "
      ...
      if (signals_.template isAttached<EFD>())
      {
        ...
        {
          Log::error() << "SynchronousMachine: efd signal attached with no linked exciter\n";
          ++error_count;
        }
      }
      ...
      if (signals_.template isAttached<PM>())
      {
        ...
        {
          Log::error() << "SynchronousMachine: pm signal attached with no linked governor\n";
          ++error_count;
        }
      }
    ...
    {
      using Variables = SynchronousMachineExternalVariables;
     ...
     */
    template <typename scalar_type, typename index_type>
    int SynchronousMachine<scalar_type, index_type>::setAbsoluteTolerance(RealT rel_tol)
    {
     ...
     */
    template <typename scalar_type, typename index_type>
    __attribute__((always_inline)) int SynchronousMachine<scalar_type, index_type>::evaluateInternalResidual(
        const ScalarT*                  y,
        const ScalarT*                  yp,
        const ScalarT*                  y_ext,
     ...
     */
    template <typename scalar_type, typename index_type>
    __attribute__((always_inline)) int SynchronousMachine<scalar_type, index_type>::evaluateExternalResidual(
        const ScalarT*                  y,
     ...
     * every attached slot through its signal node.
     */
    template <typename scalar_type, typename index_type>
    void SynchronousMachine<scalar_type, index_type>::gatherExternalVariables()
    {
      y_ext_[static_cast<size_t>(SynchronousMachineExternalVariables::EFD)] = efd_set_;
      y_ext_[static_cast<size_t>(SynchronousMachineExternalVariables::PM)]  = pm_set_;
     ...
     */
    template <typename scalar_type, typename index_type>
    int SynchronousMachine<scalar_type, index_type>::evaluateResidual()
    {
    ...
    template <typename scalar_type, typename index_type>
    const Model::VariableMonitorBase* SynchronousMachine<scalar_type, index_type>::getMonitor() const
    {
      return monitor_.get();
    }

    template <typename scalar_type, typename index_type>
    void SynchronousMachine<scalar_type, index_type>::initializeMonitor()
    {
