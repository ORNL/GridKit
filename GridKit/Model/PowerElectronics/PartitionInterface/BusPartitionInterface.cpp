...
namespace GridKit
{
  ...
  namespace PowerElectronics
  {
    ...
    /**
     * @brief Constructs a partition interface between a bus and a circuit component.
     *
     * Initializes the interface with a clone of the component across the partition
     * boundary and marks all interface variables as external. The constructor also
     * identifies the component Jacobian rows associated with the bus and stores the
     * corresponding Jacobian entries for use by the interface.
     *
     * @param bus Bus associated with the partition interface.
     * @param component Component across the partition boundary to clone.
        ...
        bus_(bus)
    {
      ...
      // Mark all interface variables as external.
      for (IdxT i = 0; i < size_; i++)
      {
        extern_indices_.insert(i);
      }

      // Collect the global indices of variables associated with the bus.
      std::vector<IdxT> bus_connections(static_cast<size_t>(bus_->size()));

      for (size_t i = 0; i < bus_connections.size(); ++i)
      {
        bus_connections[i] = bus_->getNodeConnection(i).idx_;
      }

      // Identify bus-associated Jacobian rows and collect all entries in each row.
      ...
      for (IdxT k = 0; k < component_->nnz(); ++k)
      {
        ...
        bool is_bus_row = std::find(bus_connections.begin(), bus_connections.end(), row_node) != bus_connections.end();
    ...
    /**
     * @brief Evaluates the internal residual.
     *
     * All variables are external from the interface's point of view, so there
     * are no internal residual contributions to evaluate.
     *
     * @return 0 on success.
     */
    template <class ScalarT, typename IdxT>
