...
a bus and has the following fields:
  ...
  `mon`              | Optional field, which is an array specifying variables to monitor the value of in an output channel. Available variables are determined by the node class. See the table below for more information
...
Operating inputs `p`, `q`, `online`, `tap`, `phase`, `open`, and `active` are optional. When an operating input is not explicitly
mapped to a signal, `SystemModel` supplies a constant signal initialized from
State, then from the branch parameters (`tap` and `phase`), then from the model default.
This list is subject to change.

An explicitly mapped signal takes precedence over State. Bus `bus1` is the
Branch tap side.

## Initial operating state

The solver's `state_file` names a separate state JSON file. Bus entries use
`bus_id_<number>` keys and provide `vr` and `vi`. Device entries use their case
`id` and provide operating inputs such as `p`, `q`, `online`, or `active`.
Generator and ZIP-load `p` and `q` are signed injections on the system base;
loads therefore use negative values for consumption. Explicit signal mappings
supply their own backing values.
...
```json
{
   ...
   "devices": [
       ...
       { "class": "Genrou", "ports": {"bus":1, "speed": 1, "pmech":2, "efd":3}, "id": "DV1", "params": {"H":3.0, "D":0.0, "Ra":0.0, "Tdop":7.0, "Tdopp":0.04, "Tqopp":0.05, "Tqop":0.75, "Xd":2.1, "Xdp":0.2, "Xdpp":0.18, "Xq":0.5, "Xqp": 0.0, "Xqpp":0.18, "Xl":0.15, "S10":0.0, "S12":0.0}, "mon": ["delta", "omega"] },
       ...
       { "class": "BusFault", "ports": {"bus":1}, "id": "EVT1", "params": {"R":0.0, "X":1e-3} }
   ]
}
```
