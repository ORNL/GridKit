...
namespace GridKit
{
  namespace PhasorDynamics
  {
    namespace Converter
    {
      ...
      Repca<ScalarT, IdxT>::Repca(bus_type* bus)
        : bus_(bus)
      {
      ...
      template <class ScalarT, typename IdxT>
      Repca<ScalarT, IdxT>::Repca(bus_type* bus, const model_data_type& data)
        : bus_(bus),
      ...
      template <class ScalarT, typename IdxT>
      Repca<ScalarT, IdxT>::~Repca()
      {
      }

      template <class ScalarT, typename IdxT>
      ScalarT& Repca<ScalarT, IdxT>::Vr()
      {
        return bus_->Vr();
      }

      template <class ScalarT, typename IdxT>
      ScalarT& Repca<ScalarT, IdxT>::Vi()
      {
        return bus_->Vi();
      }

      template <class ScalarT, typename IdxT>
      void Repca<ScalarT, IdxT>::setDerivedParameters()
      {
      ...
      template <class ScalarT, typename IdxT>
      ScalarT Repca<ScalarT, IdxT>::toComponentBase(ScalarT value) const
      {
        return value * system_to_component_base_;
      }

      template <class ScalarT, typename IdxT>
      int Repca<ScalarT, IdxT>::verifyBranchSignalPorts() const
      {
        int ret = 0;
      ...
      template <class ScalarT, typename IdxT>
      void Repca<ScalarT, IdxT>::initModelParams(const model_data_type& data)
      {
        ...
        loadRequiredSwitch(Params::VcompFlag, VcompFlag_, "VcompFlag");
        loadRequiredSwitch(Params::RefFlag, RefFlag_, "RefFlag");
        loadRequiredSwitch(Params::Freqflag, Freqflag_, "Freqflag");
        loadRequiredReal(Params::Tfltr, Tfltr_, "Tfltr");
        loadRequiredReal(Params::Tft, Tft_, "Tft");
        loadRequiredReal(Params::Tfv, Tfv_, "Tfv");
        loadRequiredReal(Params::Tp, Tp_, "Tp");
        loadRequiredReal(Params::Tlag, Tlag_, "Tlag");
        loadRequiredReal(Params::Vfrz, Vfrz_, "Vfrz");
        loadRequiredReal(Params::Rc, Rc_, "Rc");
        loadRequiredReal(Params::Xc, Xc_, "Xc");
        loadRequiredReal(Params::Kc, Kc_, "Kc");
        loadRequiredReal(Params::dbdlow, dbdlow_, "dbdlow");
        loadRequiredReal(Params::dbdupper, dbdupper_, "dbdupper");
        loadRequiredReal(Params::emax, emax_, "emax");
        loadRequiredReal(Params::emin, emin_, "emin");
        loadRequiredReal(Params::Kp, Kp_, "Kp");
        loadRequiredReal(Params::Ki, Ki_, "Ki");
        loadRequiredReal(Params::Qmax, Qmax_, "Qmax");
        loadRequiredReal(Params::Qmin, Qmin_, "Qmin");
        loadRequiredReal(Params::fdbd1, fdbd1_, "fdbd1");
        loadRequiredReal(Params::fdbd2, fdbd2_, "fdbd2");
        loadRequiredReal(Params::Ddn, Ddn_, "Ddn");
        loadRequiredReal(Params::Dup, Dup_, "Dup");
        loadRequiredReal(Params::femax, femax_, "femax");
        loadRequiredReal(Params::femin, femin_, "femin");
        loadRequiredReal(Params::Kpg, Kpg_, "Kpg");
        loadRequiredReal(Params::Kig, Kig_, "Kig");
      ...
      template <class ScalarT, typename IdxT>
      const Model::VariableMonitorBase* Repca<ScalarT, IdxT>::getMonitor() const
      {
        return monitor_.get();
      }

      template <class ScalarT, typename IdxT>
      void Repca<ScalarT, IdxT>::initializeMonitor()
      {
      ...
      template <class ScalarT, typename IdxT>
      int Repca<ScalarT, IdxT>::setGridKitComponentID(IdxT component_id)
      {
      ...
      template <class ScalarT, typename IdxT>
      int Repca<ScalarT, IdxT>::allocate()
      {
        setDerivedParameters();
      ...
      template <class ScalarT, typename IdxT>
      int Repca<ScalarT, IdxT>::verify() const
      {
        ...
        check(VcompFlag_ == ZERO<RealT> || VcompFlag_ == ONE<RealT>, "VcompFlag must be 0 or 1");
        check(RefFlag_ == ZERO<RealT> || RefFlag_ == ONE<RealT>, "RefFlag must be 0 or 1");
        check(Freqflag_ == ZERO<RealT> || Freqflag_ == ONE<RealT>, "Freqflag must be 0 or 1");
        ...
        check(dbdlow_ <= ZERO<RealT> && ZERO<RealT> <= dbdupper_, "dbdlow <= 0 <= dbdupper is required");
        check(emin_ <= ZERO<RealT> && ZERO<RealT> <= emax_, "emin <= 0 <= emax is required");
        check(Qmin_ <= Qmax_, "Qmin must be less than or equal to Qmax");
        check(fdbd1_ <= ZERO<RealT> && ZERO<RealT> <= fdbd2_, "fdbd1 <= 0 <= fdbd2 is required");
      ...
      template <class ScalarT, typename IdxT>
      ScalarT Repca<ScalarT, IdxT>::readExternalOrDefault(RepcaExternalVariables variable,
                                                          ScalarT                default_value) const
      {
        switch (variable)
        {
      ...
      template <class ScalarT, typename IdxT>
      int Repca<ScalarT, IdxT>::initialize()
      {
        ...
        {
          Log::error() << "Repca: regulated-bus voltage magnitude must be positive at initialization\n";
          return 1;
        }
        ...
        pplantref_set_ = readExternalOrDefault(RepcaExternalVariables::PPLANTREF, y_[PMEAS] - pfreq0);
        ...
        if (signals_.template isAssigned<RepcaInternalVariables::PEXT>() && Freqflag_ != ZERO<RealT>)
        {
      ...
      template <class ScalarT, typename IdxT>
      int Repca<ScalarT, IdxT>::tagDifferentiable()
      {
      ...
      template <class ScalarT, typename IdxT>
      __attribute__((always_inline)) inline int Repca<ScalarT, IdxT>::evaluateInternalResidual(
          ScalarT* y,
          ScalarT* yp,
          ScalarT* wb,
          ScalarT* ws,
      ...
      template <class ScalarT, typename IdxT>
      int Repca<ScalarT, IdxT>::evaluateResidual()
      {
        setDerivedParameters();
        ...
        if (signals_.template isAttached<RepcaExternalVariables::IBRANCHR>())
        {
          ws_[IBRANCHR] = signals_.template readExternalVariable<RepcaExternalVariables::IBRANCHR>();
          ws_indices_[IBRANCHR] =
              signals_.template readExternalVariableIndex<RepcaExternalVariables::IBRANCHR>();
        }
        if (signals_.template isAttached<RepcaExternalVariables::IBRANCHI>())
        {
          ws_[IBRANCHI] = signals_.template readExternalVariable<RepcaExternalVariables::IBRANCHI>();
        ...
        if (signals_.template isAttached<RepcaExternalVariables::PPLANTREF>())
        {
          ws_[PPLANTREF] = signals_.template readExternalVariable<RepcaExternalVariables::PPLANTREF>();
