...
\begin{aligned}
  ...
  I_{\mathrm{N},r} &\leftarrow G(V_d\sin(\delta)+V_q\cos(\delta))-B(-V_d\cos(\delta)+V_q\sin(\delta))\\
  I_{\mathrm{N},i} &\leftarrow B(V_d\sin(\delta)+V_q\cos(\delta))+G(-V_d\cos(\delta)+V_q\sin(\delta))\\
