...
namespace GridKit
{
  namespace PhasorDynamics
  {
    ...
    template <typename scalar_type, typename index_type>
    bool Gensal<scalar_type, index_type>::isOnline() const
    {
      static constexpr auto ONLINE = GensalSignalInputs::online;
    ...
    template <typename scalar_type, typename index_type>
    scalar_type Gensal<scalar_type, index_type>::onlineFactor() const
    {
      if (isOnline())
      {
    ...
    void Gensal<scalar_type, index_type>::initializeMonitor()
    {
      ...
      monitor_->set(Variable::ir, [this]
                    { return onlineFactor() * this->toSystemBase(y_.getData()[12]); });
      monitor_->set(Variable::ii, [this]
                    { return onlineFactor() * this->toSystemBase(y_.getData()[13]); });
      monitor_->set(Variable::p, [this]
                    { return onlineFactor() * this->toSystemBase(Vr() * y_.getData()[12] + Vi() * y_.getData()[13]); });
      monitor_->set(Variable::q, [this]
                    { return onlineFactor() * this->toSystemBase(Vi() * y_.getData()[12] - Vr() * y_.getData()[13]); });
    ...
    int Gensal<scalar_type, index_type>::initialize()
    {
      ...
      static constexpr auto P = GensalSignalInputs::p;
      static constexpr auto Q = GensalSignalInputs::q;
