  ...
  namespace PhasorDynamics
  {
    ...
    void Gensal<scalar_type, index_type>::initializeMonitor()
    {
      ...
      monitor_->set(Variable::ir, [this]
                    { return onlineFactor() * toSystemBase(y_.getData()[12]); });
      monitor_->set(Variable::ii, [this]
                    { return onlineFactor() * toSystemBase(y_.getData()[13]); });
      monitor_->set(Variable::p, [this]
                    { return onlineFactor() * toSystemBase(Vr() * y_.getData()[12] + Vi() * y_.getData()[13]); });
      monitor_->set(Variable::q, [this]
                    { return onlineFactor() * toSystemBase(Vi() * y_.getData()[12] - Vr() * y_.getData()[13]); });
