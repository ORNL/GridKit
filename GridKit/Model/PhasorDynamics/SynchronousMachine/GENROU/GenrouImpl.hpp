...
namespace GridKit
{
  namespace PhasorDynamics
  {
    ...
    int Genrou<scalar_type, index_type>::initialize()
    {
      ...
      const ScalarT psipp = std::sqrt(Vint_r * Vint_r + Vint_i * Vint_i);
      const ScalarT ksat  = SB_ * Math::qramp(psipp - SA_);
      ...
      efd_set_ = static_cast<RealT>(Eqp + Xd1_ * (s.id + Xd3_ * (Eqp - psidp - Xd2_ * s.id)) + s.psidpp * s.ksat);
