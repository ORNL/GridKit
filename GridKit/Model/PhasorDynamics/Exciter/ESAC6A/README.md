...
$T_A$                            | [s]      | `Ta`     | Regulator denominator time constant          | 0.1           |
$T_K$                            | [s]      | `Tk`     | Regulator numerator time constant            | 0.0           |
$T_B$                            | [s]      | `Tb`     | Lag time constant for second lead–lag block  | 0.0           |
$T_C$                            | [s]      | `Tc`     | Lead time constant for second lead–lag block | 0.0           |
$V_A^{\max}$                     | [p.u.]   | `VaMax`  | Maximum first regulator block output         | 1.0           | Source label: `VAMAX`
$V_A^{\min}$                     | [p.u.]   | `VaMin`  | Minimum first regulator block output         | -1.0          | Source label: `VAMIN`
...
$K_H$                            | [p.u.]   | `Kh`     | Feedback-limiter gain                        | 1.0           | Source label: `KH`
$V_H^{\max}$                     | [p.u.]   | `Vhmax`  | Maximum feedback-limiter lead–lag output     | 1.0           | Source label: `VHMAX`; lower limit is zero
$T_H$                            | [s]      | `Th`     | Feedback-limiter denominator time constant   | 0.0           | Source label: `TH`
$T_J$                            | [s]      | `Tj`     | Feedback-limiter numerator time constant     | 0.0           | Source label: `TJ`
...
Smooth functions: [`clamp`](../../../../CommonMath.md#clamp), [$q$](../../../../CommonMath.md#quadratic-ramp).
...
\begin{aligned}
  ...
  0 &= -V_H^{\mathrm{pre}}
       ...
       \end{cases} \\
  0 &= -V_H + \text{clamp}\!(V_H^{\mathrm{pre}}; 0, V_H^{\max}) \\
...
\begin{aligned}
  ...
  V_{\mathrm{FE}} &\leftarrow (K_E + S_E)V_E + K_D I_{\mathrm{fd}} + V_{\mathrm{FE}}^{\mathrm{lim}} \\
  V_F &\leftarrow V_{\mathrm{FE}} \\
  V_H^{\mathrm{pre}} &\leftarrow V_F \\
  V_H &\leftarrow \text{clamp}(V_H^{\mathrm{pre}}; 0, V_H^{\max}) \\
  V_R &\leftarrow V_{\mathrm{FE}} \\
  V_{\mathrm{LL}} &\leftarrow V_R + K_H V_H \\
  V_A &\leftarrow V_{\mathrm{LL}} \\
  x_A &\leftarrow V_A \\
  x_{\mathrm{LL}} &\leftarrow V_{\mathrm{LL}} \\
  V_C &\leftarrow E_C \\
  e_V &\leftarrow \dfrac{x_A}{K_A} \\
  V_{\mathrm{ref}} &\leftarrow e_V + V_C + V_F - V_{\mathrm{uel}} - V_S
\end{aligned}
```
