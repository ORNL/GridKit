...
namespace GridKit
{
  namespace PhasorDynamics
  {
    namespace Stabilizer
    {
      ...
      {
        size_ = static_cast<IdxT>(Utilities::enum_size<IeeestInternalVariables>());
      ...
      int Ieeest<scalar_type, index_type>::verify() const
      {
        ...
        if (!ports_.in.template port<IeeestSignalInputs::input>().connected())
        {
          Log::error() << "Ieeest: required input signal U is not attached\n";
          ret += 1;
        }
        else if (!ports_.in.template port<IeeestSignalInputs::input>().linked())
        {
