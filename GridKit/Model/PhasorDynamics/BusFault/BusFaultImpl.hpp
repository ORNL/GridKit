...
namespace GridKit
{
  namespace PhasorDynamics
  {
    ...
    int BusFault<scalar_type, index_type>::allocate()
    {
      ...
      this->allocateExternalVectors(static_cast<IdxT>(Utilities::enum_size<BusFaultExternalVariables>()));
      f_ext_.resize(2);
      f_ext_.setToZero();
     ...
     */
    template <typename scalar_type, typename index_type>
    __attribute__((always_inline)) int BusFault<scalar_type, index_type>::evaluateExternalResidual(
        const ScalarT*                  y,
    ...
    /**
     * \brief Internal residual for the fault model.
     *
     */
    template <typename scalar_type, typename index_type>
    int BusFault<scalar_type, index_type>::evaluateInternalResidual()
    {
    ...
    /**
     * \brief External residual contributions to the bus while the fault is
     * active.
     *
     */
    template <typename scalar_type, typename index_type>
    int BusFault<scalar_type, index_type>::evaluateExternalResidual()
    {
      auto* y_ext = y_ext_.getData();
