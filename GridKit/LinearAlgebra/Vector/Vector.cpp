...
namespace GridKit
{
  namespace LinearAlgebra
  {
    ...
    namespace detail
    {
      ...
      template void logBoundsCheckFailure(size_t, size_t);
      template void logBoundsCheckFailure(int, int);
      ...
      template void logUpdatedBoundsCheckFailure(long int, long int);
      template void logUpdatedBoundsCheckFailure(size_t, size_t);
      template void logUpdatedBoundsCheckFailure(int, int);
    } // namespace detail
    ...
    Vector<ScalarT, IdxT>::Vector(IdxT n)
      : n_capacity_(n),
        k_(1),
        n_size_(n),
        gpu_updated_single_(false),
        cpu_updated_single_(false)
    {
    }
                                       ...
                                       memory::MemorySpace    memspace)
    {
      ...
      // Detach from any previous parent so setData writes this vector's own flags.
      owns_update_flags_ = true;

      const int status = setData(base + offset, size, memspace);
      if (status != 0)
        return status;

      // Point at the parent's flags so the view reports the parent's freshness.
      if (parent.owns_update_flags_)
      {
        gpu_updated_ = &parent.gpu_updated_single_;
        cpu_updated_ = &parent.cpu_updated_single_;
      }
      else
      {
