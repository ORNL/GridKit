...
<br><br>
```math
I_r = \frac{PV_r + QV_i}{V_r^2 + V_i^2} \\[0.2cm]
I_i = \frac{PV_i - QV_r}{V_r^2 + V_i^2} \\[0.2cm]
E_r = \frac{ g(I_r + gV_r - bV_i) + b (I_i + bV_r + gV_i)   }{g^2 + b^2}\\[0.2cm]
E_i = \frac{ -b(I_r + gV_r - bV_i) + g (I_i + bV_r + gV_i)   }{g^2 + b^2}\\[0.2cm]
E_p = \sqrt{E_r^2 + E_i^2}\\[0.2cm]
\delta = atan2(E_i, E_r) \\[0.2cm]
\omega = 0 \\[0.2cm]
T_{elec} = gE_p^2 - E_p \bigg( \bigg(gV_r -  bV_i \bigg) \cos \delta + \bigg(bV_r + gV_i \bigg)\sin \delta\bigg)\\[0.2cm]
P_{mech} = T_{elec}
```