...
namespace PhasorDynamics
{
    ...
    Branch<ScalarT, IdxT>::Branch(bus_type* bus1, bus_type* bus2)
        : bus1_(bus1), bus2_(bus2), R_(0.0), X_(0.01), G_(0.0), B_(0.0), bus1ID_(0), bus2ID_(0)
    {
        size_ = 0;
    }
     ...
     */
    template <class ScalarT, typename IdxT>
    Branch<ScalarT, IdxT>::Branch(bus_type* bus1, bus_type* bus2, real_type R, real_type X, real_type G, real_type B)
        : bus1_(bus1), bus2_(bus2), R_(R), X_(X), G_(G), B_(B), bus1ID_(0), bus2ID_(0)
    {
    }

    template <class ScalarT, typename IdxT>
    Branch<ScalarT, IdxT>::Branch(bus_type* bus1, bus_type* bus2, BranchData& data)
        : bus1_(bus1), bus2_(bus2), R_(data.r), X_(data.x), G_(0.0), B_(data.b), bus1ID_(data.fbus), bus2ID_(data.tbus)
    {
        size_ = 0;
    }
