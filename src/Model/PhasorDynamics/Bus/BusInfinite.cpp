...
namespace PhasorDynamics
{
     ...
     */
    template <class ScalarT, typename IdxT>
    BusInfinite<ScalarT, IdxT>::BusInfinite(ScalarT Vr, ScalarT Vi) : Vr_(Vr), Vi_(Vi)
    {
        // std::cout << "Create BusInfinite..." << std::endl;
        // std::cout << "Number of equations is " << size_ << std::endl;
    ...
    BusInfinite<ScalarT, IdxT>::BusInfinite(BusData& data)
        : BusBase<ScalarT, IdxT>(data.bus_i), Vr_(data.Vm * cos(data.Va)), Vi_(data.Vm * sin(data.Va))
    {
        size_ = 0;
    }
