 ...
 * Since the line is of medium length then there is no real part for shunt
 * admittance
...
TransmissionLine<ScalarT, IdxT>::TransmissionLine(IdxT    id,
                                                  ScalarT R,
                                                  ScalarT X,
                                                  ScalarT B)
    : R_(R), X_(X), B_(B)
 ...
 *  Iim = Dre Vim + Dim Vre
 *
 * To express this for Modified Nodal Analysis the Voltages of the admittance
 * matrix are put into voltage drops
 */
template <class ScalarT, typename IdxT>
int TransmissionLine<ScalarT, IdxT>::evaluateResidual()
{
    ...
    // Internal variables
    // row 1
    f_[8] = YReMat_ * (V1re - V2re) - (YImMatDi_ * V1im + YImMatOff_ * V2im)
            - y_[8];
    f_[9] = YReMat_ * (V1im - V2im) + (YImMatDi_ * V1re + YImMatOff_ * V2re)
            - y_[9];

    // row2
    f_[10] = YReMat_ * (V2re - V1re) - (YImMatOff_ * V1im + YImMatDi_ * V2im)
             - y_[10];
    f_[11] = YReMat_ * (V2im - V1im) + (YImMatOff_ * V1re + YImMatDi_ * V2re)
             - y_[11];
