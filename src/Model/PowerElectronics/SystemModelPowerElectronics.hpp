...
public:
     ...
     *
     * @todo this should throw an exception as no allocation without a graph is
     * allowed. Or needs to be removed from the base class
     *
     * @return int
     */
    int allocate()
    ...
    /**
     * @brief Will check if each component has jacobian avalible. If one doesn't
     ...
     * @todo Add capability to go through component model connection to get the
     * size of the actual vector
     *
     * @param[in] s size of the vector
     *
     * @post System model vectors allocated with size s
     *
     * @return int 0 if successful, positive if there's a recoverable error,
     * negative if unrecoverable
     */
    int allocate(IdxT s)
    {
    ...
    /**
     * @brief Distribute y and y' to each component based of node connection
     * graph
     *
    ...
    int evaluateResidual()
    {
        ...
        for (const auto& component : components_)
        {
            ...
            for (IdxT j = 0; j < component->size(); ++j)
            {
                ...
                if (component->getNodeConnection(j) != neg1_)
                {
                    f_[component->getNodeConnection(j)] +=
                        component->getResidual()[j];
                }
            }
        }
    ...
    /**
     * @brief Creates the Sparse COO Jacobian representing  \alpha dF/dy' +
     * dF/dy
     *
     * @return int 0 if successful, positive if there's a recoverable error,
     * negative if unrecoverable
    ...
    int evaluateJacobian()
    {
        ...
        for (const auto& component : components_)
        {
            ...
            std::tuple<std::vector<IdxT>&,
                       std::vector<IdxT>&,
                       std::vector<ScalarT>&>
                tpm               = component->getJacobian().getEntries();
            ...
            {
                if (component->getNodeConnection(r[i]) != neg1_
                    && component->getNodeConnection(c[i]) != neg1_)
                {
