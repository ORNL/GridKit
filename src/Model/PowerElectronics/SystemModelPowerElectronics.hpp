...
public:
     ...
     */
    PowerElectronicsModel(double rt = 1e-4, double at = 1e-4, bool ju = false, IdxT msa = 2000)
     ...
     *
     * @todo this should throw an exception as no allocation without a graph is allowed. Or needs to be removed from the
     * base class
     *
     * @return int
     */
    ...
    int evaluateJacobian()
    {
        ...
        for (const auto& component : components_)
        {
            ...
            std::tuple<std::vector<IdxT>&, std::vector<IdxT>&, std::vector<ScalarT>&> tpm =
                component->getJacobian().getEntries();
