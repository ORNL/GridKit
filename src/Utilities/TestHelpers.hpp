...
namespace Testing
{
    ...
    public:
        ...
        TestStatus(bool success) : outcome_(success ? TestOutcome::PASS : TestOutcome::FAIL)
        {
        }

        TestStatus(const char* funcname) : outcome_(TestOutcome::PASS), funcname_(funcname)
        {
        }
        ...
        TestOutcome report(const char* funcname)
        {
            ...
            case UNEXPECTED_PASS:
                ...
                break;
            default:
                std::cout << "--- " << RED << "FAIL" << CLEAR << "Unrecognized test result " << outcome_ << " for test "
                          << funcname << "\n";
            }
            return outcome_;
        }
