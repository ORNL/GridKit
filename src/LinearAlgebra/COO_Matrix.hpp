 ...
 *
 * @todo add functionality to keep track of multiple sorted_ list. Faster adding of new entries and will have a
 * threshold to sort completely.
 *
 * m x n sparse matrix
 */
...
public:
    ...
    std::tuple<std::vector<Intdx>, std::vector<Intdx>, std::vector<ScalarT>>
    getEntrieCopiesSubMatrix(std::vector<Intdx> submap);
...
inline std::tuple<std::vector<Intdx>, std::vector<ScalarT>> COO_Matrix<ScalarT, Intdx>::getRowCopy(Intdx r)
{
    ...
    return {{this->column_indexes_.begin() + rowindex, this->column_indexes_.begin() + rsize},
            {this->values_.begin() + rowindex, this->values_.begin() + rsize}};
}
 ...
 */
template <class ScalarT, typename Intdx>
inline std::tuple<std::vector<Intdx>&, std::vector<Intdx>&, std::vector<ScalarT>&>
COO_Matrix<ScalarT, Intdx>::getEntries()
{
    if (!this->sorted_)
    {
        this->sortSparse();
...
template <class ScalarT, typename Intdx>
inline std::tuple<std::vector<Intdx>, std::vector<Intdx>, std::vector<ScalarT>>
COO_Matrix<ScalarT, Intdx>::getEntrieCopies()
{
    if (!this->sorted_)
    {
        this->sortSparse();
...
template <class ScalarT, typename Intdx>
inline std::tuple<std::vector<Intdx>, std::vector<Intdx>, std::vector<ScalarT>>
COO_Matrix<ScalarT, Intdx>::getDataToCSR()
{
    if (!this->isSorted())
        this->sortSparse();
...
inline void COO_Matrix<ScalarT, Intdx>::setValues(std::vector<Intdx> r, std::vector<Intdx> c, std::vector<ScalarT> v)
{
    ...
    for (Intdx i = 0; i < static_cast<Intdx>(this->row_indexes_.size()); i++)
    {
        ...
        while (aiter < static_cast<Intdx>(r.size())
               && (r[aiter] < this->row_indexes_[i]
                   || (r[aiter] == this->row_indexes_[i] && c[aiter] < this->column_indexes_[i])))
        {
...
template <class ScalarT, typename Intdx>
inline void
COO_Matrix<ScalarT, Intdx>::axpy(ScalarT alpha, std::vector<Intdx> r, std::vector<Intdx> c, std::vector<ScalarT> v)
{
    if (alpha == 0)
        return;
 ...
 */
template <class ScalarT, typename Intdx>
inline void COO_Matrix<ScalarT, Intdx>::permutationSizeMap(std::vector<Intdx> row_perm,
                                                           std::vector<Intdx> col_perm,
                                                           Intdx              m,
                                                           Intdx              n)
{
    assert(row_perm.size() == this->rows_size_);
 ...
 */
template <class ScalarT, typename Intdx>
inline Intdx COO_Matrix<ScalarT, Intdx>::sparseCordBinarySearch(const std::vector<Intdx>& rows,
                                                                const std::vector<Intdx>& columns,
                                                                Intdx                     ri,
                                                                Intdx                     ci)
{
 ...
 */
template <class ScalarT, typename Intdx>
inline void COO_Matrix<ScalarT, Intdx>::sortSparseCOO(std::vector<Intdx>&   rows,
                                                      std::vector<Intdx>&   columns,
...
inline COO_Matrix<ScalarT, Intdx>::COO_Matrix(
    std::vector<Intdx> r, std::vector<Intdx> c, std::vector<ScalarT> v, Intdx m, Intdx n)
{
    this->values_         = v;
    this->row_indexes_    = r;
