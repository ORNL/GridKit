 ...
 *
 * @todo add functionality to keep track of multiple sorted lists. Faster adding of new entries and will have a
    ...
    for (IdxT i = 0; i < static_cast<IdxT>(this->row_indices_.size()); i++)
    {
        ...
        while (a_iter < static_cast<IdxT>(r.size())
               && (r[a_iter] < this->row_indices_[i]
                   || (r[a_iter] == this->row_indices_[i] && c[a_iter] < this->column_indices_[i])))
        {
...
template <class ScalarT, typename IdxT>
inline void
COO_Matrix<ScalarT, IdxT>::axpy(ScalarT alpha, std::vector<IdxT> r, std::vector<IdxT> c, std::vector<ScalarT> v)
{
    if (alpha == 0)
        return;
...
template <class ScalarT, typename IdxT>
inline void
COO_Matrix<ScalarT, IdxT>::permutationSizeMap(std::vector<IdxT> row_perm, std::vector<IdxT> col_perm, IdxT m, IdxT n)
{
    assert(row_perm.size() == this->rows_size_);
    assert(col_perm.size() == this->columns_size_);
...
inline void COO_Matrix<ScalarT, IdxT>::printMatrix(std::string name)
{
    ...
    {
        std::cout << "(" << this->row_indices_[i] << ", " << this->column_indices_[i] << ", " << this->values_[i]
                  << ")\n";
    }
    std::cout << std::flush;
}
 ...
 */
template <class ScalarT, typename IdxT>
inline IdxT COO_Matrix<ScalarT, IdxT>::sparseCordBinarySearch(const std::vector<IdxT>& rows,
 ...
 */
template <class ScalarT, typename IdxT>
inline void COO_Matrix<ScalarT, IdxT>::sortSparseCOO(std::vector<IdxT>&    rows,
...
inline COO_Matrix<ScalarT, IdxT>::COO_Matrix(
    std::vector<IdxT> r, std::vector<IdxT> c, std::vector<ScalarT> v, IdxT m, IdxT n)
{
    this->values_         = v;
    this->row_indices_    = r;
