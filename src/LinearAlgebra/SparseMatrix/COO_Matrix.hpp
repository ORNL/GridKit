...
/**
 * @brief Quick class to provide sparse matrices of COO type. Simplifies data
 * movement
 *
 * @todo add functionality to keep track of multiple sorted lists. Faster adding
 * of new entries and will have a threshold to sort completely.
 *
 * m x n sparse matrix
 */
...
public:
    ...
    std::tuple<std::vector<IdxT>&, std::vector<IdxT>&, std::vector<ScalarT>&>
    getEntries();
    std::tuple<std::vector<IdxT>, std::vector<IdxT>, std::vector<ScalarT>>
    getEntryCopies();
    std::tuple<std::vector<IdxT>, std::vector<IdxT>, std::vector<ScalarT>>
    getEntryCopiesSubMatrix(std::vector<IdxT> submap);
    ...
    void
    setValues(std::vector<IdxT> r, std::vector<IdxT> c, std::vector<ScalarT> v);
    void    axpy(ScalarT alpha, COO_Matrix<ScalarT, IdxT>& a);
    void    axpy(ScalarT              alpha,
                 std::vector<IdxT>    r,
                 std::vector<IdxT>    c,
                 std::vector<ScalarT> v);
    ...
    void permutationSizeMap(std::vector<IdxT> row_perm,
                            std::vector<IdxT> col_perm,
                            IdxT              m,
                            IdxT              n);
    ...
    static void sortSparseCOO(std::vector<IdxT>&    rows,
                              std::vector<IdxT>&    columns,
                              std::vector<ScalarT>& values);

private:
    IdxT indexStartRow(const std::vector<IdxT>& rows, IdxT r);
    IdxT sparseCordBinarySearch(const std::vector<IdxT>& rows,
                                const std::vector<IdxT>& columns,
                                IdxT                     ri,
                                IdxT                     ci);
 ...
 */
template <class ScalarT, typename IdxT>
inline std::tuple<std::vector<IdxT>, std::vector<ScalarT>>
COO_Matrix<ScalarT, IdxT>::getRowCopy(IdxT r)
{
    if (!this->sorted_)
    {
        this->sortSparse();
    ...
    return {{this->column_indices_.begin() + row_index,
             this->column_indices_.begin() + rsize},
 ...
 * @tparam ScalarT
 * @tparam IdxT
 * @return std::tuple<std::vector<IdxT>, std::vector<IdxT>,
 * std::vector<ScalarT>>
 */
template <class ScalarT, typename IdxT>
inline std::tuple<std::vector<IdxT>&, std::vector<IdxT>&, std::vector<ScalarT>&>
COO_Matrix<ScalarT, IdxT>::getEntries()
{
    if (!this->sorted_)
    {
        this->sortSparse();
...
template <class ScalarT, typename IdxT>
inline std::tuple<std::vector<IdxT>, std::vector<IdxT>, std::vector<ScalarT>>
COO_Matrix<ScalarT, IdxT>::getEntryCopies()
{
    if (!this->sorted_)
    {
        this->sortSparse();
...
template <class ScalarT, typename IdxT>
inline std::tuple<std::vector<IdxT>, std::vector<IdxT>, std::vector<ScalarT>>
COO_Matrix<ScalarT, IdxT>::setDataToCSR()
{
    if (!this->isSorted())
        this->sortSparse();
 ...
 *
 * @todo swap this with having the matrix store the data and updates. This can
 * then be passed by reference
 *
 *
 * @tparam ScalarT
 * @tparam IdxT
 ...
 */
template <class ScalarT, typename IdxT>
inline void COO_Matrix<ScalarT, IdxT>::setValues(std::vector<IdxT>    r,
                                                 std::vector<IdxT>    c,
                                                 std::vector<ScalarT> v)
{
    // sort input
    ...
    // Duplicated with axpy. Could replace with function depdent on lambda
    // expression
    IdxT a_iter = 0;
    ...
    for (IdxT i = 0; i < static_cast<IdxT>(this->row_indices_.size()); i++)
    {
        ...
        while (a_iter < static_cast<IdxT>(r.size())
               && (r[a_iter] < this->row_indices_[i]
                   || (r[a_iter] == this->row_indices_[i]
                       && c[a_iter] < this->column_indices_[i])))
        {
        ...
        if (r[a_iter] == this->row_indices_[i]
            && c[a_iter] == this->column_indices_[i])
        {
 ...
 */
template <class ScalarT, typename IdxT>
inline void COO_Matrix<ScalarT, IdxT>::axpy(ScalarT                    alpha,
                                            COO_Matrix<ScalarT, IdxT>& a)
{
    if (alpha == 0)
    {
        return;
 ...
 * @param[out] col_perm
 *
 * @pre row_perm.size() == this->rows_size_ = col_perm.size() ==
 * this->columns_size_
 *
 * @post this = this(row_perm, col_perm)
 */
template <class ScalarT, typename IdxT>
inline void COO_Matrix<ScalarT, IdxT>::permutation(std::vector<IdxT> row_perm,
                                                   std::vector<IdxT> col_perm)
{
 ...
 *
 * @post this = this(row_perm, col_perm) and removed indices have corresponding
 * values set to 0
 */
template <class ScalarT, typename IdxT>
inline void COO_Matrix<ScalarT, IdxT>::permutationSizeMap(
    std::vector<IdxT> row_perm, std::vector<IdxT> col_perm, IdxT m, IdxT n)
{
    assert(row_perm.size() == this->rows_size_);
    assert(col_perm.size() == this->columns_size_);
    ...
    for (int i = 0; i < this->values_.size(); i++)
    {
        if (row_perm[this->row_indices_[i]] == -1
            || col_perm[this->column_indices_[i]] == -1)
        {
            this->values_[i] = 0;
        }
        else
 ...
 * @post this = I_n
 *
 * @todo - it might be better to explicitly zero out the matrix and require to
 * be so in preconditions
 */
...
{
    this->sortSparseCOO(
        this->row_indices_, this->column_indices_, this->values_);
    this->sorted_ = true;
}
...
inline void COO_Matrix<ScalarT, IdxT>::printMatrix(std::string name)
{
    ...
    for (size_t i = 0; i < this->values_.size(); i++)
    {
        std::cout << "(" << this->row_indices_[i] << ", "
                  << this->column_indices_[i] << ", " << this->values_[i]
                  << ")\n";
    }
    std::cout << std::flush;
}
...
template <class ScalarT, typename IdxT>
inline IdxT
COO_Matrix<ScalarT, IdxT>::indexStartRow(const std::vector<IdxT>& rows, IdxT r)
{
 ...
 */
template <class ScalarT, typename IdxT>
inline IdxT COO_Matrix<ScalarT, IdxT>::sparseCordBinarySearch(
    const std::vector<IdxT>& rows,
    const std::vector<IdxT>& columns,
    IdxT                     ri,
    IdxT                     ci)
{
 ...
 *
 * @pre rows, columns, and values are of the same size and represent a COO
 * matrix with no duplicates
...
template <class ScalarT, typename IdxT>
inline void
COO_Matrix<ScalarT, IdxT>::sortSparseCOO(std::vector<IdxT>&    rows,
                                         std::vector<IdxT>&    columns,
                                         std::vector<ScalarT>& values)
{
    ...
    std::size_t         n(0);
    std::generate(
        std::begin(ordervec), std::end(ordervec), [&] { return n++; });
...
template <class ScalarT, typename IdxT>
inline COO_Matrix<ScalarT, IdxT>::COO_Matrix(std::vector<IdxT>    r,
                                             std::vector<IdxT>    c,
                                             std::vector<ScalarT> v,
                                             IdxT                 m,
                                             IdxT                 n)
{
    ...
    this->rows_size_      = m;
    this->columns_size_   = n;
    this->sorted_ = false; // Set to false until explicitly sorted, though
                           // logically it is sorted.
}
