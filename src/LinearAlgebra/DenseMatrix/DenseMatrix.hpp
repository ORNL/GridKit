 ...
 *
 * This is intended for small matrices that store model Jacobians to be
 * subsequently copied into large sparse matrices.
 */
namespace GridKit
{
namespace LinearAlgebra
{
    ...
    public:
        ...
        // Purposefully not defining BLAS operations. This class should not be
        // used for compute.
    };
     ...
     */
    template <class ScalarT, typename IdxT>
    DenseMatrix<ScalarT, IdxT>::DenseMatrix(const IdxT rows_size,
                                            const IdxT columns_size)
     ...
     */
    template <class ScalarT, typename IdxT>
    inline ScalarT DenseMatrix<ScalarT, IdxT>::getValue(const IdxT i,
                                                        const IdxT j) const
    {
     ...
     */
    template <class ScalarT, typename IdxT>
    inline void DenseMatrix<ScalarT, IdxT>::setValue(const IdxT    i,
                                                     const IdxT    j,
                                                     const ScalarT value)
    {
        assert(i < this->columns_size_);
    ...
    template <class ScalarT, typename IdxT>
    inline void
    DenseMatrix<ScalarT, IdxT>::setValues(COO_Matrix<ScalarT, IdxT> values_COO)
    {
        std::
            tuple<std::vector<IdxT>&, std::vector<IdxT>&, std::vector<ScalarT>&>
                entries                 = values_COO.getEntries();
