...
\foreach \k/\y in {1/0, 2/-1.6, K/-3.9}
    \node[block] (R\k) at (0, \y) {$\dfrac{\mathbf{R}^{\k}}{s - p_{\k}}$};
\node at (0, -2.75) {$\vdots$};
...
\foreach \b in {D, E, R2, RK}
    \draw[->, line] (split) |- (\b.west);

% Fan-in to summation
\node[sum] (S) at (2.6, 0) {$\Sigma$};
\draw[->, line] (R1.east) -- (S);
\foreach \b/\a in {D/north, E/north, R2/south, RK/south}
...
\node[sig, above=2pt] at (0.3, 4.2) {$\mathbf{H}(s)$};
