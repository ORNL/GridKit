...
def _data_enums(index: _Index, compound: ET.Element) -> dict[str, str | None]:
    """Resolve concrete data structs and ComponentData template aliases."""
    alias = next(
        member for member in _members(compound, "typedef")
        if _text(member.find("name")) == "ModelDataT"
    )
    ref = alias.find("./type/ref")
    if ref is None:
        raise InventoryError("ModelDataT has no documented type reference")
    if ref.get("kindref") == "compound":
        data = index.compound(ref.attrib["refid"])
        return {name: _alias(data, name, "member") for name in ALIASES}

    data = index.member(ref.attrib["refid"])
    base = data.find("./type/ref[@kindref='compound']")
    if base is None or _text(base) != "ComponentData":
        raise InventoryError("ModelDataT does not alias ComponentData")
    enums = [
        ref.attrib["refid"] for ref in data.findall("./type/ref[@kindref='member']")
        if index.member(ref.attrib["refid"]).get("kind") == "enum"
    ]
    if len(enums) != len(ALIASES):
        raise InventoryError("ComponentData must provide five documented enum types")
    return dict(zip(ALIASES, enums, strict=True))
...
def _items(
    ...
    if enum.get("kind") != "enum":
        raise InventoryError(f"{enum_refid} does not name a documented enum")
...
def read_models(directory: Path) -> dict[str, Model]:
    ...
    for refid, compound in index.compounds.items():
        ...
        if not (_inherits(index, refid, f"{NAMESPACE}::Component")
                or _inherits(index, refid, BUS_BASE)):
            continue
        if not any(_text(member.find("name")) == "ModelDataT"
                   for member in _members(compound, "typedef")):
            continue
        enums = _data_enums(index, compound)
        ...
        sections = {
            field: _items(index, enums[alias], defaults if field == "parameters" else {})
