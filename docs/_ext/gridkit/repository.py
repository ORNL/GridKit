...
class Case:
    ...
    def oneline(self) -> Path | None:
        return next(
            iter(sorted(p for p in self.path.parent.iterdir()
                        if p.suffix in {".png", ".jpg", ".svg"})), None
        )
...
def _read_case(root: Path, path: Path, pages: Path) -> Case:
    ...
    slug = kebab(path.name.removesuffix(".case.json"))
    page = pages / relative.parent / "README.md"
    if relative.parent.name == "Toy":
        page = pages / relative.parent / f"{slug}.md"
    return Case(
        path=path,
        domain=domain,
        slug=slug,
