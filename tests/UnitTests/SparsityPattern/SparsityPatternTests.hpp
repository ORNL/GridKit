...
namespace Testing
{
    ...
    public:
        ...
        TestOutcome variable()
        {
            ...
            {
                const Sparse::Variable::DependencyMap& dependencies = (f[0]).getDependencies();
            ...
            {
                const Sparse::Variable::DependencyMap& dependencies = (f[2]).getDependencies();
    ...
    private:
        template <typename T>
        void residualFunction(std::vector<T>& f, const std::vector<T>& x, const std::vector<T>& p)
        {
            const T u = x[0] * x[1];
