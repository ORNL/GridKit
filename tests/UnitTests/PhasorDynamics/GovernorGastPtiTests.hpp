  ...
  namespace Testing
  {
    ...
    public:
      ...
      TestOutcome initializationAndSignals()
      {
        ...
        success       *= scalarPreserved(fixture.output(SignalOutputs::pmech), 0.4, "preserved pmech seed");

        success *= scalarPreserved(fixture.input(External::speed),
                                   0.0,
        ...
        for (const RealT omega : speed_cases)
        {
          ...
          if (!speed_fixture.setInput(External::speed, omega)
          ...
          success             *= scalarPreserved(speed_fixture.output(SignalOutputs::pmech),
                                     initial_pmech,
                                     "signed-speed pmech preservation");
          success             *= scalarPreserved(speed_fixture.input(External::speed),
                                     omega,
      ...
      TestOutcome initializationDomain()
      {
        ...
        success *= over_rated.checkStateRows({{Internal::XVALVE, 1.2},
                                              ...
                                              {Internal::VLV, 1.2}},
                                             "over-rated dispatch");
        success *= scalarPreserved(over_rated.output(SignalOutputs::pmech),
                                   0.6,
    ...
    private:
      ...
      {
        return {
            .inputs = {{External::speed, 0.02}, {External::pref, 0.31}},
            .state  = {
      ...
      bool aliasedSignalsRejected() const
      {
        ...
        pref_alias.getPorts().in.template port<External::pref>().connect(&pmech_pref);
        ...
        speed_alias.getPorts().in.template port<External::speed>().connect(&pmech_speed);
        ...
        input_alias.getPorts().in.template port<External::speed>().connect(&shared_input);
        input_alias.getPorts().in.template port<External::pref>().connect(&shared_input);
                                            ...
                                            RealT       omega = ZERO<RealT>) const
      {
        ...
        if (!fixture.setInput(External::speed, omega)
