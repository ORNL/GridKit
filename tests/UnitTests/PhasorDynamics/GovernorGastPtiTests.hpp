  ...
  namespace Testing
  {
    ...
    public:
      ...
      TestOutcome baseConversion()
      {
        ...
        success *= isEqual(model.y()[index(Var::VTEMP)], vtemp0, scalar(kTolerance));
        success *= isEqual(model.y()[index(Var::VLV)], xflow0, scalar(kTolerance));
        success *= isEqual(model.y()[index(Var::PMECH)], pmech0, scalar(kTolerance));
      ...
      TestOutcome residual()
      {
        ...
        const std::vector<ScalarT> expected     = {
            ...
            -vlv + selected_vlv,
            -scalar(kSystemBase) * pmech + scalar(kTrate) * (xflow - scalar(kDturb) * omega_value),
        };
    ...
    private:
      ...
      static constexpr RealT kSystemBase = 100.0;
