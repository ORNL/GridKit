...
namespace GridKit
{
  namespace Testing
  {
      ...
      TestOutcome excitationLimits()
      {
        ...
        // field-voltage-state row independently of either Jacobian backend.
        {
          ...
          const DepVar::DependencyMap expected{{
              {2 * static_cast<size_t>(Internal::EFDP), -6.0},
              {2 * static_cast<size_t>(Internal::EFDP) + 1, -0.5},
              {2 * static_cast<size_t>(Internal::VR), 0.5},
              {2 * static_cast<size_t>(Internal::VFE), -0.5},
          }};
    ...
    private:
      ...
      {
        fixture.input(External::speed) = 0.05;
