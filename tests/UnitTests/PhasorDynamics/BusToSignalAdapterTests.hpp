  ...
  namespace Testing
  {
    ...
    public:
      ...
      TestOutcome residual()
      {
        ...
        adapter.getPorts().out[SignalOut::vr].connect(&vr_sig);
        adapter.getPorts().out[SignalOut::vi].connect(&vi_sig);
        adapter.getPorts().in[SignalIn::ir].connect(&ir_sig);
        adapter.getPorts().in[SignalIn::ii].connect(&ii_sig);
        adapter.allocate();
