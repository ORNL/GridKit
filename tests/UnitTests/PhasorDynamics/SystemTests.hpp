...
namespace GridKit
{
  namespace Testing
  {
    ...
    public:
      ...
      TestOutcome monitorOutputConsistency()
      {
        TestStatus success = true;
        ...
        data.Vr0  = RealT{1.25};
        data.Vi0  = RealT{-2.5};
        data.monitored_variables.insert(Variable::Vr);
        data.monitored_variables.insert(Variable::Vm);

        PhasorDynamics::Bus<ScalarT, IdxT> bus(data);
        bus.allocate();
        bus.initialize();
        ...
        monitor.addVariable("aux", &aux);
        monitor.addMonitor(bus.getMonitor());

        auto strip_final_newline = [](std::string value)
        {
        ...
        auto state_preserved = [](auto&& print)
        {
          ...
          return os.precision() == precision && os.flags() == flags;
        };

        auto check_format = [&](auto fmt)
        {
          std::ostringstream direct_header;
          monitor.printHeader(direct_header, fmt);
          std::ostringstream full_header;
          monitor.printFullHeader(full_header, fmt);
          success *= direct_header.str() == strip_final_newline(full_header.str());

          std::ostringstream direct_record;
          monitor.print(direct_record, fmt);
          std::ostringstream full_record;
          monitor.printFull(full_record, fmt);
          success *= direct_record.str() == strip_final_newline(full_record.str());

          std::ostringstream direct_footer;
          monitor.printFooter(direct_footer, fmt);
          std::ostringstream full_footer;
          monitor.printFullFooter(full_footer, fmt);
          success *= direct_footer.str() == full_footer.str();

          success *= state_preserved([&](std::ostream& os)
                                     { monitor.printHeader(os, fmt); });
          success *= state_preserved([&](std::ostream& os)
                                     { monitor.print(os, fmt); });
          success *= state_preserved([&](std::ostream& os)
                                     { monitor.printFooter(os, fmt); });
        };

        check_format(Model::VariableMonitorBase::Csv{});

        auto json = Model::VariableMonitorBase::Json{};
        check_format(json);
        json.after_first = true;
        check_format(json);

        check_format(Model::VariableMonitorBase::Yaml{});
