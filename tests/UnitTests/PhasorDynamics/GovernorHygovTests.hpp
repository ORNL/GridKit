  ...
  namespace Testing
  {
    ...
    public:
      ...
      TestOutcome validation()
      {
        ...
        success *= (configured.model().size() == static_cast<IdxT>(Utilities::enum_size<Internal>()));
      ...
      TestOutcome initializationAndSignals()
      {
        ...
        success       *= scalarMatches(fixture.output(SignalOutputs::pmech), 0.4, "preserved pmech value");
      ...
      TestOutcome initializationDomain()
      {
        ...
        success          *= scalarMatches(adjusted.output(SignalOutputs::pmech), 0.144, "preserved pmech value");
        ...
        // the prior success.
        if (!adjusted.setInput(External::speed, 0.03))
          return TestStatus(false).report(__func__);
        ...
        if (!adjusted.setInput(External::speed, 0.0))
          return TestStatus(false).report(__func__);
        success *= adjusted.checkSteadyState();
        ...
        success *= scalarMatches(lower_edge.output(SignalOutputs::pmech),
                                 p_min - 0.5 * kTol,
                                 "clipped pmech value");
      ...
      TestOutcome initializationExactness()
      {
        ...
        for (const auto& [gate, power] : points)
        {
          ...
          success               *= scalarMatches(fixture.output(SignalOutputs::pmech), power, "preserved power");
          success               *= fixture.checkSteadyState();
        }
        return success.report(__func__);
      }
      ...
      TestOutcome governorControl()
      {
        ...
        const std::array<ResidualCase, 3> deadband_cases{{
            {"below the deadband",
             {.inputs = {{External::speed, -3.0 * band}}, .state = {{Internal::OMEGADB, 0.0}}},
             {{Internal::OMEGADB, -3.0 * band}}},
            {"inside the deadband",
             {.inputs = {{External::speed, band / 4.0}}, .state = {{Internal::OMEGADB, 0.0}}},
      ...
      TestOutcome turbineDynamics()
      {
        ...
        success                *= scalarMatches(curve.output(SignalOutputs::pmech), 0.54, "preserved power");
    ...
    private:
      ...
      using SignalOutputs = PhasorDynamics::Governor::HygovSignalOutputs;
      ...
      void configureHygov(Fixture<T>& fixture, RealT system_va_base = 100.0e6) const
      {
        ...
        fixture.template assignOutput<SignalOutputs::pmech>();
      }

      template <typename T>
      void attachInputs(Fixture<T>& fixture) const
      {
        fixture.template attachInput<External::speed>(0.0);
      ...
      {
        return {
            .inputs = {{External::speed, 0.02}, {External::pref, 0.31}, {External::paux, 0.07}},
            .state  = {
                {Internal::XN, 0.11},
                {Internal::XF, 0.23},
                {Internal::C, 0.52},
      ...
      bool unlinkedSignalRejected() const
      {
        ...
        configureHygov(fixture);
        fixture.model().getPorts().in.template port<variable>().connect(&unlinked_node);
