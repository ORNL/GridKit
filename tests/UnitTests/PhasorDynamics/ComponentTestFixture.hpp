...
namespace GridKit
{
  namespace Testing
  {
    ...
    public:
      ...
      using ExternalT = typename ModelT::ModelDataT::SignalInputs;
      using OutputT = typename ModelT::ModelDataT::SignalOutputs;
      using Outputs = std::vector<std::pair<OutputT, RealT>>;
      ...
      {
        static_assert(Utilities::contained_within(variable));
        auto& input = inputs_[static_cast<size_t>(variable)];
        input.value = value;
        input.index = rowCount() + static_cast<IdxT>(variable);
        input.node.link(&input.value, &input.index);
        model_.getPorts().in.template port<variable>().connect(&input.node);
      }

      template <OutputT variable>
      void assignOutput()
      {
        static_assert(Utilities::contained_within(variable));
        model_.getPorts().out.template port<variable>().connect(
      ...
      RealT output(OutputT variable) const
      {
      ...
      Snapshot snapshot() const
      {
        ...
        for (size_t row = 0; row < outputs_.size(); ++row)
        {
          if (outputs_[row].linked())
            result.outputs.emplace_back(static_cast<OutputT>(row), output(static_cast<OutputT>(row)));
        }
        return result;
      }
    ...
    private:
      ...
      static constexpr size_t INTERNAL_COUNT = Utilities::enum_size<InternalT>();
      static constexpr size_t EXTERNAL_COUNT = Utilities::enum_size<ExternalT>();
      ...
      std::array<SignalNodeT, Utilities::enum_size<OutputT>()> outputs_{};
      ModelT                                  model_;
      std::string                             context_;
      RealT                                   tolerance_;
