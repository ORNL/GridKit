      ...
      TestOutcome jacobian()
      {
        ...
        for (const RealT alpha : {0.0, 0.5, 2.0})
        {
          for (const RealT signal : {-1.25, -1.0, 0.25, 1.0, 1.25})
          {
            ...
            // Verify automatic CSR assembly against the independent two-pass rows.
            Fixture<DepVar> automatic(makeData());
            success *= (automatic.model.initialize() == 0);
            setStatePoint(automatic, signal);
            auto* automatic_y  = automatic.model.y().getData();
            auto* automatic_yp = automatic.model.yp().getData();
            for (IdxT i = 0; i < automatic.model.size(); ++i)
            {
              const auto index = static_cast<size_t>(automatic.model.getVariableIndex(i));
              automatic_y[i].setVariableNumber(2 * index);
              automatic_yp[i].setVariableNumber(2 * index + 1);
            }
            automatic.input.setVariableNumber(2 * static_cast<size_t>(automatic.input_index));
            automatic.model.updateTime(0.0, alpha);
            success *= (automatic.model.evaluateResidual() == 0);
            success *= (automatic.model.evaluateJacobian() == 0);
            const auto assembled = MapFromCsr(automatic.model.getCsrJacobian());
            success *= (assembled.size() == rows.size());
            for (size_t row = 0; row < std::min(assembled.size(), rows.size()); ++row)
            {
              success *= isEqual(assembled[row], rows[row].getDependencies(), tol);
            }
