  ...
  namespace Testing
  {
    ...
    public:
      ...
      TestOutcome initializationAndSignals()
      {
        ...
        success       *= scalarMatches(f[index(Vars::IP)], 0.5, "latched active-current rate", smoothTol());
        success       *= scalarMatches(f[index(Vars::IQ)], 0.3, "latched reactive-current rate", smoothTol());
      ...
      TestOutcome residualEquations()
      {
        ...
        {
          success *= scalarMatches(f[index(row.variable)], row.value, row.name, smoothTol());
        }
      ...
      TestOutcome highVoltageManagement()
      {
        ...
        // Initialization above Vhvmax activates HVRCM and preserves Q0.
        {
          ...
          success                   *= scalarMatches(
              extra_current, kHvrcmGain * hvrcmOffset(), "IQEXTRA with the default Khv", smoothTol());
          success *= scalarMatches(
    ...
    private:
      ...
      {
        return static_cast<RealT>(std::numeric_limits<RealT>::digits) * hvrcmTransition();
      }
