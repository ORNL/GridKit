  ...
  namespace Testing
  {
    ...
    public:
      ...
      TestOutcome initializesFromPowerFlowAndPublishesSignals()
      {
        ...
        const ScalarT lvacm  = Math::linseg(vt, static_cast<RealT>(0.4), static_cast<RealT>(0.9), ONE<RealT>);
  ...
  "devices": [
    ...
    private:
      ...
      std::vector<ScalarT> expectedResidualForState(ScalarT ipcmd, ScalarT iqcmd) const
      {
        ...
        std::vector<ScalarT> expected(static_cast<size_t>(Vars::MAXIMUM), static_cast<ScalarT>(0.0));
...
#ifdef GRIDKIT_ENABLE_ENZYME
      void setResidualStateDep(PhasorDynamics::Converter::Regca<DependencyTracking::Variable, IdxT>& regca,
