...
namespace GridKit
{
  namespace Testing
  {
    ...
    public:
      ...
      TestOutcome signalJunctionAndForcedOscillationParse()
      {
        using namespace GridKit::PhasorDynamics;
        ...
        const char data[] = R"({
          "header": {
            "format_version": 0.2,
            "format_revision": 0,
            "case_name": "Signal junction test",
            "case_description": "Signal junction and source parser coverage",
            "case_comments": ""
          },
          "params": {
            "freq_base": 60.0,
          ...
          "buses": [],
          "signals": [
            {"signal_id": 1, "name": "original"},
            {"signal_id": 2, "name": "forcing"},
            {
              "signal_id": 3,
              "name": "junction",
              "junction": {
        ...
        success *= result.signal.size() == 3;
        success *= result.signal[2].junction.has_value();
        if (result.signal[2].junction.has_value())
        {
          const auto& junction  = *result.signal[2].junction;
          success              *= junction.bias == 0.25;
          success              *= junction.initialization_input == 1;
          success              *= junction.inputs.size() == 2;
          success              *= junction.inputs[0].signal_id == 1;
          success              *= junction.inputs[0].gain == 1.0;
          success              *= junction.inputs[1].signal_id == 2;
          success              *= junction.inputs[1].gain == -2.0;
        }

        using SourceData  = typename SystemModelData<RealT, IdxT>::ForcedOscillationDataT;
        success          *= result.forced_oscillation.size() == 1;
        if (result.forced_oscillation.size() == 1)
        {
          const auto& source  = result.forced_oscillation[0];
          success            *= source.disambiguation_string == "fo";
          success            *= std::get<RealT>(source.parameters.at(SourceData::Parameters::A)) == 0.1;
          success            *= std::get<RealT>(source.parameters.at(SourceData::Parameters::f)) == 0.7;
          success            *= source.signal_outputs.at(SourceData::SignalOutputs::output) == 2;
          success            *= source.monitored_variables.contains(
              SourceData::MonitorableVariables::output);
          success *= source.monitored_variables.contains(
              SourceData::MonitorableVariables::envelope);
          success *= source.monitored_variables.contains(
              SourceData::MonitorableVariables::active);
        }
