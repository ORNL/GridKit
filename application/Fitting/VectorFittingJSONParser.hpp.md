    ...
    namespace Application
    {
      namespace Detail
      {
        ...
        inline std::ifstream openJsonFile(const fs::path& file_path)
        {
          ...
          {
            throw std::runtime_error("VectorFitting solver file not found: " +
                                     file_path.string());
          }
          ...
          if (!stream)
          {
            throw std::runtime_error(
                "Failed to open VectorFitting solver file: " +
                file_path.string());
          }
          return stream;
        }
        ...
        inline void rejectUnsupportedVocabulary(const json& j)
        {
          for (const auto* key : {"frequency_hz", "hz", "weights_column"})
          {
            ...
            {
              throw std::runtime_error(
                  "VectorFitting solver JSON does not support \"" +
                  std::string(key) + "\"");
            }
          }
        }
        ...
        inline Weighting parseWeighting(const std::string& name)
        {
          ...
          throw std::runtime_error("Invalid VectorFitting weighting: " +
                                   name);
        }
      } // namespace Detail
      ...
      parseVectorFittingData(const std::filesystem::path& solver_file)
      {
        ...
        if (j.contains("fit"))
        {
          ...
          if (data.fit.max_poles != 0 &&
              (data.fit.min_poles < 1 ||
               data.fit.max_poles < data.fit.min_poles))
          {
            throw std::runtime_error(
        ...
        if (!data.output_state_space.empty() &&
            !data.output_state_space.is_absolute())
        {
